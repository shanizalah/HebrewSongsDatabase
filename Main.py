...
for index, record in enumerate(allRecords):
            ...
            if tag == "100" or tag == "110" or tag == "700" or tag == "710":  # multiple field
                ...
                for item in items:
                    ...
                    if code == 'a':
                        ...
                        if role == "מלחין":
                            if not person == '':
                                if person not in composersList:
                                    composersList.append(person)
                                '''
                        ...
                        if role == "מחבר":
                            if not person == '':
                                if person not in poetsList:
                                    poetsList.append(person)
                                '''
                        ...
                        if role == "singer" or role == "זמר" or role == "performer" or role == "מבצע":
                            if not person == '':
                                if person not in performersList:
                                    performersList.append(person)
                                '''
                        ...
                        if role == "מעבד מוזיקלי":
                            if not person == '':
                                if person not in musicalAdapterList:
                                    musicalAdapterList.append(person)
                                '''
                ...
                if not flage:
                    if not person == '':
                        if person not in othersList:
                            othersList.append(person)
                        '''
                ...
                for item in items:
                    code = item[u'@code']
                    if code == 'a':
                        entityEntry["SongName"] = (cleanStrSongName(item['#text'].strip()))
    ...
    if performersList:
        entityEntry["Performer"] = str(cleanStr1(listToString(performersList)))
    if composersList:
        entityEntry["Composer"] = str(cleanStr1(listToString(composersList)))
    if poetsList:
        entityEntry["Poet"] = str(cleanStr1(listToString(poetsList)))
    if musicalAdapterList:
        entityEntry["MusicalAdapter"] = str(cleanStr1(listToString(musicalAdapterList)))
    if othersList:
        entityEntry["Others"] = str(cleanStr1(listToString(othersList)))
